...
function ChessBoard({ setGameState }: ChessBoardProps) {
  ...
  useEffect(() => {
    ...
    if (isCheckMate) 
      // last letter means which player lost
      setGameState(`checkmate_${chessGame.turn()}`)
    else if (isDraw) 
      setGameState('draw')
    else if (isStalemate) 
  ...
  function getMoveOptions(square: Square) {
    ...
    for (const move of moves) {
      newSquares[move.to] = {
        // If move.to sqaure is occupied by opposite color, show larger circle else show smaller circle
  ...
  //FIXME: temroparary off dragging because it's lagging
    // onPieceDrop,
    allowDragging: false,
    onSquareClick,
